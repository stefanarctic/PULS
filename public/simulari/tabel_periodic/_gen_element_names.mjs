import fs from "fs";
const txt = fs.readFileSync(new URL("./elements.js", import.meta.url), "utf8");
/** @type {string[]} */
const syms = [...txt.matchAll(/symbol: "([^"]+)"/g)].map((m) => m[1]);
...
const EN = {
  ...
  Sn: "Tin", Sb: "Antimony", Te: "Tellurium", I: "Iodine", Xe: "Xenon", Cs: "Caesium", Ba: "Barium", La: "Lanthanum",
  ...
  Ra: "Radium", Ac: "Actinium", Th: "Thorium", Pa: "Protactinium", U: "Uranium", Np: "Neptunium", Pu: "Plutonium",
...
const out = {};
for (const s of syms) {
  if (!EN[s]) throw new Error("Missing EN for " + s);
  out[s] = EN[s];
}
console.log("count", Object.keys(out).length);
fs.writeFileSync(new URL("./_element_names_preview.json", import.meta.url), JSON.stringify(out, null, 0));
